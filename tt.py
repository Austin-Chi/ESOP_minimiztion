...
from sympy import symbols, Add, Poly

# Define the variables
x, y, z = symbols('x y z')

# Define your polynomial or expression
expression = x*y*z**2 + x**2*y*z + x*y**3*z**2 + x*y*z + x**5
print(type(expression))
# expression = Poly(expression)
# Separate the terms in the expression
terms = Add.make_args(expression)
...
# f = 3 * x**3 + 2 * x**2 + x * y + y**5 + 1
# order = 2

# # coeffs = Poly(f, x).coeffs()
# coeffs_2 = Poly(f, y).coeffs()
# # f_new = sum(x**n * coeffs[-(n+1)] for n in range(order+1)) # the +1 is to get 0th order
# f_new_2 = sum(y**n * coeffs_2[-(n+1)] for n in range(order+1))
# # print(f_new)
# print(f_new_2)
# # 2*x**2 + x*y + y**3 + 1