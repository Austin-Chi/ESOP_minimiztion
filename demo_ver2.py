...
for idx, g in enumerate(gates):
    ...
    if numctrl == 0:
        current_x[tar_bit] = Xor(current_x[tar_bit], 1)
        print('current: ',current_x)
    elif numctrl == 1:
        current_x[tar_bit] = Xor(current_x[tar_bit], current_x[ctr_bits[0]])
        print('current: ',current_x)
    else:
        product = 1
        for bit in ctr_bits:
           product = And(product, current_x[bit])
        current_x[tar_bit] = Xor(current_x[tar_bit], product)
print('circuit done.')
# print(current_x)
...
for bit in range(numbits):
    if garbage[bit] == '-':
        ...
        cost_original = 0
        current_output = [sym for sym in polynomials]
        for sym in output:
            current_output.append(sym)
        # print(output)
        print(current_output)
        # Compute the Grobner basis
        GB = groebner(current_output, vars, order='grlex')
